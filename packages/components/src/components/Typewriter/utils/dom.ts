 ...
 * @param moveHandler
 */
export function keyMove(trigger: HTMLElement, moveHandler?: (left: number, top: number) => void): (() => void) {
 ...
 * @param checkKey 全局名称
 */
export function appendHandler<K extends keyof HTMLElementTagNameMap>(tagName: K, attributes: Partial<HTMLElementTagNameMap[K]>, checkKey = '') {
 ...
 * @param value 属性值
 */
export function updateHandler<K extends keyof HTMLElementTagNameMap>(tagName: K, attributes: Partial<HTMLElementTagNameMap[K]>) {
  const insertedEle = document.getElementById(attributes.id!)
  insertedEle?.remove()
 ...
 * @returns
 */
export function createHTMLElement<K extends keyof HTMLElementTagNameMap>(tagName: K, attributes: Partial<HTMLElementTagNameMap[K]>): HTMLElementTagNameMap[K] {
...
export const zoomMermaid = (() => {
  ...
  const handler = (
    ...
    console.log('zoomMermaid-handler-options', options)
    ...
    containers?.forEach((mm) => {
      ...
      if (!actionDiv) {
        mm.insertAdjacentHTML(
          'beforeend',
          `<div class="${prefix}-mermaid-action">${svgMap.pinOff}</div>`,
        )
      ...
      const onClick = () => {
        ...
        if (current?.removeEvent) {
          ...
          actionDiv.innerHTML = svgMap.pinOff
        }
        else {
          // 添加事件并记录
          ...
          actionDiv.innerHTML = svgMap.pin
        }
      }
